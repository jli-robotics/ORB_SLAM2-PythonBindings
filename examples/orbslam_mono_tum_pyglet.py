...
def pyglet_update(dt, window):
    window.animate()

def plot(slam):
    global window
    kfs = slam.get_keyframe_points()
    map_points = slam.get_all_map_points()
...
if __name__ == '__main__':
    ...
    window = Viz()
    x = threading.Thread(target=main, args=(sys.argv[1], sys.argv[2], sys.argv[3]))
    x.start()
    pyglet.clock.schedule_interval(pyglet_update, .5, window)
    pyglet.app.run()
